...
export function BrandKitsSection() {
  ...
  const serializeLogos = useCallback(
    (currentLogos: LocalAsset[]) =>
      currentLogos.flatMap((currentLogo) => {
        ...
        if (!url) return [];
        return [{ id: currentLogo.id, name: currentLogo.name.trim() || inferLogoNameFromUrl(url), url }];
      }),
    [],
  );

  const computeSnapshot = useCallback(() => {
    return JSON.stringify({ brand, promptConfig, logos: serializeLogos(logos) });
  ...
  const saveSettings = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/settings", {
        ...
        body: JSON.stringify({ brand, promptConfig, logoUrl: primaryLogoUrl }),
      });
      ...
      if (activeKitId) {
        const kitRes = await fetch(`/api/brand-kits/${activeKitId}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ name: kitName, brand, promptConfig, logos: persistedLogos, logoUrl: primaryLogoUrl }),
        });
  ...
  const handleLogoUpload = async (event: ChangeEvent<HTMLInputElement>) => {
        ...
        try {
          ...
          setLogos((current) =>
            current.map((existingLogo) => {
              if (existingLogo.id !== stagedLogo.id) return existingLogo;
              revokeObjectUrlIfNeeded(existingLogo.previewUrl);
              return { ...existingLogo, previewUrl: url, storageUrl: url, status: "uploaded", error: undefined };
            }),
          );
        } catch (uploadError) {
          failureCount += 1;
          setLogos((current) =>
            current.map((existingLogo) =>
              existingLogo.id === stagedLogo.id
                ? { ...existingLogo, status: "local", error: uploadError instanceof Error ? uploadError.message : "Upload failed" }
                : existingLogo,
            ),
          );
        }
    ...
    if (failureCount === 0) { toast.success(files.length === 1 ? "Logo uploaded." : "Logos uploaded."); return; }
    if (failureCount === files.length) { toast.error(files.length === 1 ? "Logo upload failed." : "Logo uploads failed."); return; }
          ...
          <div className="space-y-6 p-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <Button
                ...
                disabled={isSaving || !isDirty || hasUnsyncedLogos}
                size="sm"
              >
            ...
            <div className="rounded-3xl border border-white/15 bg-slate-900/55 p-5 backdrop-blur-xl md:p-6">
              ...
              <div className="mb-5">
                <div className="mb-1.5 flex items-center justify-between gap-3">
                  ...
                  <Button type="button" variant="outline" size="xs" onClick={() => logoInputRef.current?.click()}>
                ...
                >
                  ...
                  <span className="text-[11px] text-slate-400">Each logo gets its own editable display name.</span>
                </button>
                <input
                  id={logoInputId}
                  ref={logoInputRef}
                ...
                {logos.length > 0 ? (
                  <div className="mt-3 space-y-3">
                    {logos.map((logo, index) => (
                      <div key={logo.id} className="flex items-start gap-3 rounded-xl border border-white/10 bg-black/20 p-3">
                        <div className="flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-lg border border-white/15 bg-black/30 p-2">
                          ...
                          <img src={logo.previewUrl} alt={`${logo.name || `Logo ${index + 1}`} preview`} className="max-h-full max-w-full object-contain" />
                        </div>
                        <div className="min-w-0 flex-1 space-y-2">
                          <div className="flex items-center justify-between gap-2">
                            <span className={cn("rounded-full border px-2.5 py-1 text-[11px] font-medium", statusChip(logo.status))}>
                              {logo.status === "uploading" ? "Uploading" : logo.error ? "Needs attention" : "Ready"}
                            </span>
                            <Button
                              type="button"
                              variant="ghost"
                              ...
                              onClick={() =>
                                setLogos((current) => {
                                  const removingLogo = current.find((existingLogo) => existingLogo.id === logo.id);
                                  if (removingLogo) revokeObjectUrlIfNeeded(removingLogo.previewUrl);
                                  return current.filter((existingLogo) => existingLogo.id !== logo.id);
                                })
                              }
                          ...
                          <div className="space-y-1">
                            <Label htmlFor={`logo-name-${logo.id}`} className="text-[11px] text-slate-300">Logo name</Label>
                            <Input
                              ...
                              onChange={(event) =>
                                setLogos((current) =>
                                  current.map((existingLogo) =>
                                    existingLogo.id === logo.id ? { ...existingLogo, name: event.target.value } : existingLogo,
                                  ),
                                )
                              }
                          ...
                          <p className="text-[11px] text-slate-400">
                            {logo.error || (logo.status === "uploading" ? "Syncing to storage..." : logo.storageUrl)}
                          </p>
                        </div>
                      </div>
                    ))}
                  ...
                  <p className="mt-2 text-[11px] text-slate-400">No logos uploaded yet.</p>
                )}
                {hasUnsyncedLogos ? (
                  <p className="mt-2 text-[11px] text-amber-200">Wait for uploads to finish or remove failed logos before saving.</p>
                ) : null}
              </div>
