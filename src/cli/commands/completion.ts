...
_ig() {
  ...
  case "$command" in
    ...
    generate)
      ...
      ;;
    photos)
      COMPREPLY=( $(compgen -W "recent search import --since --limit --media --favorite --album --ids --folder" -- "$cur") )
      ;;
    publish)
  ...
  command)
    case $words[2] in
      ...
      generate)
        ...
        ;;
      photos)
        _values 'photos command' recent search import --since --limit --media --favorite --album --ids --folder
        ;;
      publish)
...
const buildFishCompletion = () => [
  ...
  "complete -c ig -n '__fish_seen_subcommand_from photos' -a 'recent search import --since --limit --media --favorite --album --ids --folder'",
