...
- The current Apple Photos slice covers local `.app` bundle installation through `scripts/install-companion-bridge.zsh`, bridge-backed native-app launch, native pick/export, localhost import handoff, PhotoKit-backed `recent` / `search` enumeration for CLI + MCP, and CLI/MCP `import` handoff into the standard remote asset upload path. It still does not cover signed distribution, hardened local bridge auth, or the higher-level `ig photos pick|propose` flows yet.
