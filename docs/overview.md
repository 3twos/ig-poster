...
- New generated layouts are now fit against the current overlay copy using deterministic safe-zone stacking, and the preview can now re-measure rendered canonical blocks outside editor mode to push a safer restack upstream when the live DOM proves the copy is taller than estimated. The editor still exposes an `Auto-fit Layout` recovery action for existing drafts, that action now prefers live measured block heights when the canvas has them, and the canvas surfaces explicit layout warnings when canonical blocks overlap or spill outside the frame.
