...
- The current Apple Photos slice covers local `.app` bundle installation through `scripts/install-companion-bridge.zsh`, bridge-backed native-app launch, native pick/export, localhost import handoff, PhotoKit-backed `recent` / `search` enumeration for CLI + MCP, CLI/MCP `import` handoff into the standard remote asset upload path, plus both `ig photos pick` and `ig photos propose` for human-driven draft ingestion/generation. It still does not cover signed distribution or hardened local bridge auth.
