...
- The CLI preview now exposes local Apple Photos enumeration/import/propose through `ig photos recent` / `ig photos search` / `ig photos import` / `ig photos propose`, and `ig mcp` now mirrors that capability with `photos_recent` / `photos_search` / `photos_import` / `photos_propose` tools for agents on macOS.
...
- The current Apple Photos slice covers local `.app` bundle installation through `scripts/install-companion-bridge.zsh`, bridge-backed native-app launch, native pick/export, localhost import handoff, PhotoKit-backed `recent` / `search` enumeration for CLI + MCP, CLI/MCP `import` handoff into the standard remote asset upload path, and a first `ig photos propose` / `photos_propose` draft-generation flow that scores local assets, creates a draft, and triggers generation. It still does not cover signed distribution, hardened local bridge auth, or the higher-level human-oriented `ig photos pick` flow yet.
