...
- The CLI preview now exposes local Apple Photos enumeration/import/pick/propose through `ig photos recent` / `ig photos search` / `ig photos import` / `ig photos pick` / `ig photos propose`, while `ig mcp` mirrors the agent-facing enumeration/import/propose capability with `photos_recent` / `photos_search` / `photos_import` / `photos_propose` tools for agents on macOS.
...
- Apple Photos support is now functionally shipped for internal and developer use behind the checked-in macOS companion scaffold. The direction remains a web-first flow with a macOS companion app plus local bridge for Photos-only access, while the web app and versioned API stay the source of truth for auth, uploads, posts, generation, and publish state. If the companion is missing, the product falls back to install guidance plus the normal upload flow instead of blocking the user.
- The current Apple Photos slice covers local `.app` bundle installation through `scripts/install-companion-bridge.zsh`, bridge-backed native-app launch, native pick/export, localhost import handoff, PhotoKit-backed `recent` / `search` enumeration for CLI + MCP, CLI/MCP `import` handoff into the standard remote asset upload path, plus both `ig photos pick` and `ig photos propose` for human-driven draft ingestion and generation. The bridge now keeps the default browser-compatible port `43123` alive even when the installer is pointed at a custom primary port. Remaining work is productization: signed distribution, packaged install/discovery, and hardened local bridge auth.
