...
- The current Apple Photos slice covers local `.app` bundle installation through `scripts/install-companion-bridge.zsh`, bridge-backed native-app launch, native pick/export, localhost import handoff, PhotoKit-backed `recent` / `search` enumeration for CLI + MCP, CLI/MCP `import` handoff into the standard remote asset upload path, plus both `ig photos pick` and `ig photos propose` for human-driven draft ingestion/generation. The bridge now keeps the default browser-compatible port `43123` alive even when the installer is pointed at a custom primary port. Signed distribution and hardened local bridge auth are still pending.
