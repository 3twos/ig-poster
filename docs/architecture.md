...
8. Browser queue/planner reads can request a best-effort Facebook schedule sync first, importing compatible scheduled Page posts from Meta into local remote-authoritative shadow jobs before listing them.
9. Page webhook route (`POST /api/webhooks/meta/page`) can trigger that same Facebook shadow-job reconciliation path for matching connected Page accounts, so Meta-side schedule changes do not rely only on planner loads.
10. Cron route (`GET /api/cron/publish`) first fails stale `processing` jobs for manual review, then claims due queued jobs, enforces the rolling 24-hour Meta publish-window guardrail, resolves auth, publishes app-managed jobs, and reconciles remote-authoritative Facebook shadow jobs against Meta before transitioning each job (`published`, deferred back to `queued`, or retry/`failed`).
