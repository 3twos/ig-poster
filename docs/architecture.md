...
- UI layer:
  ...
  - `src/components/poster-section.tsx` now also derives and surfaces canonical layout warnings when blocks overlap or extend beyond the frame, so the editor has a deterministic signal alongside the `Auto-fit Layout` control.
...
- Application-service layer:
  ...
  - `src/services/instagram-sync.ts` owns best-effort Instagram published-state reconciliation for stale or incomplete local destination metadata when the app already has an IG media id to query.
...
- Domain layer (`src/lib/*`):
  ...
  - canonical overlay geometry diagnostics that detect overlap/out-of-bounds issues for the hook/headline/body/CTA stack and feed that signal into the editor UI
...
1. Client submits caption + media payload to `POST /api/meta/schedule` (plus optional first comment, optional location metadata, optional user tags, optional reel `shareToFeed` control, and a browser-only publish target of `instagram`, `facebook`, or `both`).
...
4. If `publishAt` is >2 minutes in the future, Instagram routes store an app-managed scheduled job in Postgres (`publish_jobs`), while Facebook routes create the remote Page schedule first and then store a local shadow job with the returned remote identifiers. When the browser targets `both`, the route fans out into both destination-specific schedule paths from the same request.
5. Otherwise route publishes immediately through Meta Graph API helpers, again fanning out to separate destination executions when the browser target is `both`.
6. If provided, optional first-comment text is posted right after publish (`/{media-id}/comments`) for both immediate and cron-driven Instagram publishes. Optional `locationId` is passed through for Instagram feed/reel publish jobs, optional top-level `userTags` are passed through for Instagram single-image and reel jobs, and carousel image-item tags are passed through per child item. Optional reel `shareToFeed` is passed through for reel-mode publish jobs. In `both` mode, those Instagram-only metadata fields are omitted from the Facebook branch while unsupported shared media (for example carousels) are rejected up front.
7. Successful publish marks the linked post `posted`, appends publish history, and leaves that post immutable from the app's post-edit APIs. Instagram publishes also do a best-effort Graph read for the published media permalink/timestamp so the post snapshot and destination state keep the remote publish metadata.
8. Browser and `/api/v1` post-detail reads now do a best-effort Instagram published-state refresh for posted posts when the app has an Instagram media id but the local destination metadata is stale or incomplete.
9. Browser queue/planner reads can request a best-effort Facebook schedule sync first, importing compatible scheduled Page posts from Meta into local remote-authoritative shadow jobs before listing them.
10. Page webhook route (`POST /api/webhooks/meta/page`) can trigger that same Facebook shadow-job reconciliation path for matching connected Page accounts, so Meta-side schedule changes do not rely only on planner loads.
11. Cron route (`GET /api/cron/publish`) first fails stale `processing` jobs for manual review, then claims due queued jobs, enforces the rolling 24-hour Meta publish-window guardrail, resolves auth, publishes app-managed jobs, and reconciles remote-authoritative Facebook shadow jobs against Meta before transitioning each job (`published`, deferred back to `queued`, or retry/`failed`).
...
- Core post CRUD can still fall back to the older `posts.publishSettings`-only path when `post_destinations` or its enums are missing in a local/outdated database, but that fallback is intentionally limited to draft resilience; destination-aware scheduling/publishing still expects the destination migrations to be present.
- Instagram immediate publishes still enforce the rolling 24-hour quota reservation in `src/lib/publish-jobs.ts`; Facebook jobs bypass that Instagram-specific guard while keeping the same destination-aware queue metadata. Successful Instagram publishes now also project published permalink/timestamp data back into `posts.publishHistory` and `post_destinations`, while remote-authoritative Facebook schedules are stored locally as shadow jobs so planner/queue views can stay aligned with Meta, supported remote mutations flow back through the shared publish-job service, and queue/planner loads can import compatible scheduled Page posts back from Meta.
