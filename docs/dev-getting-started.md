...
- `src/db/schema.ts`: Drizzle ORM schema for `posts`, `brand_kits`, `publish_jobs`, `meta_accounts`, and `post_destinations` tables (including ordered named brand-kit logos, persisted `mediaComposition` and `publishSettings` on posts, optional `first_comment`, `location_id`, and `user_tags` publish metadata fields, plus destination/account-pair metadata that underpins the Meta multi-destination re-architecture).
